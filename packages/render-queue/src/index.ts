...
export class PostgresSceneRenderQueue implements SceneRenderQueue {
    ...
    const result = await this.pool.query<RenderRow>(
      `WITH authorized_scene AS (
       ...
       ON CONFLICT (story_id, scene_id, input_hash) DO UPDATE SET
         ...
         updated_at = now()
       RETURNING id, profile_id, story_id, scene_id, input_hash, input, status, storage_key, size_bytes, content_hash, created_at, error`,
  ...
  async listAuthorized(profileId: string, storyId: string, sceneId: string): Promise<readonly SceneRenderJob[]> {
    const result = await this.pool.query<RenderRow>(
      `SELECT id, profile_id, story_id, scene_id, input_hash, input, status, storage_key, size_bytes, content_hash, created_at, error
  ...
  async claim(workerId: string, leaseMilliseconds: number): Promise<SceneRenderJob | undefined> {
    const result = await this.pool.query<RenderRow>(
      `WITH candidate AS (
       ...
       RETURNING r.id, r.profile_id, r.story_id, r.scene_id, r.input_hash, r.input, r.status, r.storage_key, r.size_bytes, r.content_hash, r.created_at, r.error`,
  ...
  async complete(renderId: string, workerId: string, storageKey: string, sizeBytes: number, contentHash: string): Promise<boolean> {
    const result = await this.pool.query(
      `UPDATE scene_renders SET status = 'ready', storage_key = $3, size_bytes = $4, content_hash = $5, error = NULL,
