...
export interface RenderDependency {
  readonly role: "original" | "image-edit" | "video-track" | "audio-track";
...
export function sceneRenderParameters(input: SceneRenderInput): Record<string, unknown> {
  ...
  if (input.rendererId === "still-image") return {
    ...renderer, durationSeconds: input.durationSeconds, motion: input.motion, focusPoint: input.focusPoint,
    source: { width: input.material.width, height: input.material.height, orientation: input.material.orientation },
    output: input.output,
  };
  return {
    ...renderer, mode: input.mode, trim: input.edit.trim ?? null, sourceDurationSeconds: input.sourceDurationSeconds ?? null,
    ...(input.mode === "audio" ? {} : {
