...
Локальная очередь из десяти task/interface-пар больше не ведётся. Когда агент
разработки завершает и закрывает issue из product roadmap, он перед завершением
своей задачи запускает subagent для обновления приоритетов. Subagent заново
определяет текущий milestone, читает полные bodies, зависимости, labels и
acceptance criteria всех его открытых issues и выбирает три задачи, которые
сильнее всего приближают проверяемый продуктовый результат milestone: сначала
critical path и прямую пользовательскую ценность, затем dependency leverage,
снижение риска и только после этого value/effort.

Subagent заменяет repository pinned issues через уже подключённый GitHub-доступ;
отдельные GitHub Actions, API keys и внешние очереди для этого не используются.
Если открытых задач меньше трёх, pin-ится доступное количество. Pins показывают
текущий фокус планирования, но не меняют status, milestone или критерии
завершения issue. Этот процесс выполняется в рамках активной задачи агента и сам
по себе не запускается от закрытия issue человеком непосредственно на GitHub.
