  ...
  const response = await api.inject({
    ...
    payload: {
      ...
      events: [{
        ...
        event_type: "scene title changed",
        event_properties: { surface: "story-web", title_change_kind: "appearance" },
        device_id: "device-1", user_id: "profile-1",
      }, {
...
test("rejects events and properties outside the analytics taxonomy", async (context) => {
  ...
  const invalidSceneTitleChangeKind = await api.inject({
    method: "POST", url: "/analytics/amplitude",
    payload: {
      api_key: projectKey,
      events: [{
        event_type: "scene title changed", device_id: "device-1",
        event_properties: { surface: "story-web", title_change_kind: "raw-title-text" },
      }],
    },
  });
