...
import { extractAudioWaveform, type MediaProcessRunner } from "@storyteller/renderer";
...
export async function readMaterialWaveform(material: VideoMaterial, objects: ObjectStorage, runner: MediaProcessRunner): Promise<number[]> {
  ...
  try {
    ...
    return await extractAudioWaveform({
      sourcePath, pcmPath: join(directory, "audio.pcm"), durationSeconds: material.sourceDurationSeconds,
    }, runner);
  } finally {
