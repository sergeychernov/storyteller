...
export function SceneInspector({ scene, copy, saving, panel, onChange }: SceneInspectorProps) {
  ...
  return (
    <div className="scene-settings-panel">
      {panel === "layout" && <section><div className="inspector-heading"><div><h2>{copy.layout}</h2><small>{copy.layoutHint}</small></div><code>{materialOrientationSequence(scene.materials) || "—"}</code></div>
