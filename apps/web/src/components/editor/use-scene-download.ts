...
import {
  downloadSceneRender, getSceneRender, requestSceneRender, type AuthSession, type Scene,
} from "../../api.js";
import type { EditorCopy } from "./editor-copy.js";
import { SceneRenderTimeoutError, waitForSceneRender } from "./scene-render-polling.js";

interface DownloadFile { readonly url: string; readonly filename: string; readonly sceneVersion: string }
...
export function useSceneDownload(scene: Scene, storyId: string, session: AuthSession, copy: EditorCopy) {
  ...
  async function download() {
    ...
    setError(undefined);
    try {
      const initial = await requestSceneRender(session.accessToken, storyId, scene.id, signal);
      ...
      const prepared = {
        ...
        filename: `${safeFileName(scene.title) || `scene-${scene.id}`}.mp4`,
        sceneVersion,
      };
      setFile(prepared);
      saveFile(prepared);
